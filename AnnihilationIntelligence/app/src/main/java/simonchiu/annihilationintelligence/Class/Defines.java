package simonchiu.annihilationintelligence.Class;

/**
 * Created by Simon on 12/02/2017.
 */

//A Defines class to define certain words as integer values to make the code more readable
//This includes any maths functions or other definitions which is used in the code

public final class Defines {
    //Defines for the options
    public static final int MUSIC = 0;
    public static final int SOUND = 1;
    public static final int INVERTX = 2;
    public static final int INVERTY = 3;
    public static final int ORIENTATION = 4;

    //Defines for the activity flow to/from the PauseMenu Activity
    public static final int OPTIONS = 0;
    public static final int GAME = 1;

    //Defines for music names
    public static final int MUSIC_MENU = 0;
    public static final int MUSIC_GAME = 1;
    public static final int MUSIC_OPTIONS = 2;

    //Define for sound names
    public static final int SOUND_SELECT = 0;
    public static final int SOUND_FAIL = 1;
    public static final int SOUND_LOCKED = 2;
    public static final int SOUND_PICKUP = 3;
    public static final int SOUND_STAIR = 4;
    public static final int SOUND_ELEV_DING = 5;
    public static final int SOUND_ELEV_DOORS = 6;
    public static final int SOUND_CODE = 7;
    public static final int SOUND_WIN = 8;
    public static final int SOUND_DEATH = 9;

    //Defines for maths
    public static final float DEG_TO_RAD = 3.141f / 180f;
}
